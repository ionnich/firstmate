...
- `SECONDMATE_SYNC: secondmate home <path>: project registry for <project> converged to primary posture: <old-line> -> <new-line>` - the secondmate's `data/projects.md` entry for `<project>` had drifted from the primary's registered posture (for example a missing `+yolo`) and was just corrected; informational, no action needed unless the captain did not expect that posture change.
