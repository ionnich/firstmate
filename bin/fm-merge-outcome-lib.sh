...
# Optional <authority> is yolo, away-grant, autonomous-dispatch, attended, or
# external. Every authority except attended is appended to the ledger line;
...
fm_merge_outcome_report() {  # <home> <state> <task-id> <pr-url> <origin> [authority]
  ...
  case "$authority" in
    yolo|away-grant|external|autonomous-dispatch) suffix=" $authority" ;;
    attended|'') ;;
    *) return 2 ;;
  esac
