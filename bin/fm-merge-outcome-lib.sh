...
# Optional <authority> is away, attended, autonomous-dispatch, or external (the
# retired yolo and away-grant values are still accepted for a persisted
# authority written before the words model landed). Away, external, an
# autonomous dispatch, and the retired tags are appended to the ledger line;
# attended remains untagged. The merge entrypoint supplies its authority after
# forge acceptance, while the poll supplies the persisted identity-bound value
# or external when no matching record proves that this home authorized the
# merge.
#
...
fm_merge_outcome_report() {  # <home> <state> <task-id> <pr-url> <origin> [authority]
  ...
  case "$authority" in
    away|external|yolo|away-grant|autonomous-dispatch) suffix=" $authority" ;;
    attended|'') ;;
    *) return 2 ;;
  esac
