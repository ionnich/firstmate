...
quarantine_shared() {
  ...
  case "$REL" in data/captain-shared.md|data/captain-opinions.md) ;; *) return 0 ;; esac
  [ -f "$DEST" ] || return 0
...
case "$COMMAND" in
  put)
    ...
    if [ -f "$DEST" ] && cmp -s "$TMP" "$DEST"; then
      case "$REL" in data/captain-shared.md|data/captain-opinions.md) chmod 444 "$DEST" ;; esac
      printf 'unchanged: %s\n' "$REL"
      exit 0
    fi
