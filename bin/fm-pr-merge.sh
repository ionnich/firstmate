...
# state/.afk-contract exists, a merge for this task proceeds under away
# authority only if its meta records yolo=on or its id is in that record's
# merge-grant list; otherwise it is held for the captain's return. An
# unreadable record refuses rather than being skipped, neither posture
# releases a captain hold, and a grant lapses when the record is archived.
...
require_current_away_authority() {
  ...
  fm_lease_forbid_branch "PR merge (fm-pr-merge)" --away-relocated
  require_autonomous_dispatch_grant || resolve_merge_authority || return 1
