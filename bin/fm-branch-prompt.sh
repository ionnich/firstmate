...
- `bin/fm-pr-merge.sh`: a merge the words call for proceeds only when that task's meta records yolo=on or its id is in the record's merge-grant list, green at its live head, synchronously, under the record lock; which pull request the words meant is your reading, and a green task without either is held for the captain's return.
