...
A dormant mate is still scanned, but its stale rows are dropped rather than escalated because a deliberately stopped mate's watcher cannot advance its queue.
...
A dormant mate (`state/<id>.dormant`, owned by [`bin/fm-secondmate-dormancy-lib.sh`](../bin/fm-secondmate-dormancy-lib.sh)) is deliberately stopped and is skipped rather than relaunched.
