...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  captain-opinions.md main-authoritative advisory stances propagated read-only to secondmate homes; LOCAL, gitignored, never grants authority
...
Treat `data/captain.md` as the domain-local record of captain preferences, optional `data/captain-shared.md` as the main-authoritative shared captain-preference file, optional `data/captain-opinions.md` as advisory primary-authoritative stances that never grant authority, and `data/learnings.md` as curated home-local knowledge, regardless of harness memory.
...
7. **Context digest and next step** - last of the bulk sections, the full contents of `data/projects.md`, `data/secondmates.md`, `data/captain.md`, `data/captain-shared.md`, `data/captain-opinions.md`, and `data/learnings.md`, each clearly delimited, followed by the closing reminder.
...
- Vague captain stances belong in the primary home's advisory-only `data/captain-opinions.md`; exact instructions and native authority always outrank them.
...
- `finks-ddd` - load before routing or sequencing a Linear-linked request, before filing a ticket-linked backlog item, and before a terminal outcome on a ticket-linked task.
