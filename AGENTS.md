...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  captain-opinions.md primary-local advisory stances for vague judgment; LOCAL, gitignored, read on demand and never authority
...
state/               runtime records and signals; gitignored
  ...
  <id>.dormant       durable dormancy marker for a deliberately stopped secondmate; bin/fm-secondmate-dormancy-lib.sh owns its format and lifecycle
...
2. **Bootstrap** - detect-only checks (tool/version problems, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   The secondmate liveness sweep deterministically accounts for every registered secondmate: it leaves a dormant mate (state/<id>.dormant) untouched, relaunches only from the recovery-grade `dead` or `missing` states, preserves ambiguous, unreadable, or unreachable remote targets, and reports skipped or failed guarantees as `SECONDMATE_LIVENESS:` lines (`bin/fm-bootstrap.sh`; `bin/fm-backend.sh`'s `fm_backend_agent_state`; `docs/remote-secondmates.md`).
